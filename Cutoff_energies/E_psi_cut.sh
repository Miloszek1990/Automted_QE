...
rm $TITLE"_RESULTS_1_phys.txt"
rm $TITLE"_RESULTS_2_comp.txt"
RESULTS_1=$TITLE"_RESULTS_1_phys.txt"
RESULTS_2=$TITLE"_RESULTS_2_comp.txt"
...
WAVE_CUT=$(seq $psi_cut_beg $psi_step $psi_cut_end)	
echo $range" SCF calculations for yours system will be done."
echo "Test starts at "$psi_cut_beg" Ry and ends at "$psi_cut_end" Ry, with "$psi_step" Ry sampling."
if [ "$USPP" = "true" ]
then
...
TITLE_TMP=$TITLE"_"$psi_cut_beg"-"$i"-"$psi_cut_end"_"$FUNC
